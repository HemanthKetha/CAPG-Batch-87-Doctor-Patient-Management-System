...
int book_appointment()
{
	...
	while(1)
	{
		...
		{
			if(strcmp(str,ptr->specialization)==0)
			{
		...
		if(ptr)
		{
			...
			strcpy(new->pat_name,s1);
			new->aan=aan;
			strcpy(new->book_slot,s);
		//	new->visit_no=new->visit_no+s1_count;
			new->license_no=ptr->MCI_no;

			printf("\n Doctor Name:%s\n MCI Number:%d\n Slot:%s\n",new->doc_name,new->license_no,new->book_slot);
